...
def test_verify_passes_without_package(tmp_path):
    ...
    assert "[PASS] rmclite recovery" in p.stdout
...
def test_verify_cross_checks_the_package(package_home, tmp_path):
    env = {**os.environ, "RMCPROFILE_HOME": package_home, "PYTHONIOENCODING": "utf-8"}
    p = subprocess.run([sys.executable, VERIFY, "--minutes", "3"], capture_output=True, text=True, cwd=str(tmp_path), env=env)
    assert p.returncode == 0, p.stdout + p.stderr
    assert "[PASS] package smoke test" in p.stdout and "[PASS] package cross-check" in p.stdout
